...
буду о них подробно рассказывать, так как статья не об этом. Если вы хотите
больше узнать о веб-компонентах, я рекомендую обратить внимание на
...
Что касается веб-компонентов, на помощь приходят два фреймворка,
...
предоставляя также и декларативный стиль.
...
Всё, что нужно для использования созданного элемента — это импортировать его:
...
Этот код —  все, что нужно для создания собственных элементов с помощью Polymer.
...
как нативные элементы. Если не вдаваться в подробности, нет никакой разницы
...
Новым здесь является использование элемента `<content>`, с помощью него
...
`ngTransclude`. Точки вставки, на самом деле, более сложно устроены, но я сейчас не буду
...
повнимательней. Во первых, это `<shadow>` элемент. `<shadow>` — очень мощный, так
...
`extends="foo bar"`, верно? К сожалению, решения нет. Чтобы сделать понятнее:

**Невозможно расширить несколько элементов**

Вы ничего не найдете в официальной документации, а так же можете почитать об
этом на [StackOverflow][7]. Однако, есть несколько способов получить
