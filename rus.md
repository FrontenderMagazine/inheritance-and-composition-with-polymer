...
больше узнать о веб-компонентах, рекомендую обратить внимание на
сообщество [webcomponents.org][1], в нем есть много полезных ресурсов и
информации об этих технологиях.

С развитием стандартов также происходит развитие инструментов, библиотек и фреймворков.
Что касается веб-компонентов, тут на помощь приходят два фреймворка,
которые пытаются облегчить вашу жизнь: [X-Tag][2] от Mozilla и [Polymer][3]
...
императивный способ создания компонентов, в то время как Polymer идет несколько
дальше, предлагая также и декларативный стиль.
...
Заглянув немного глубже, мы сможем увидеть все четыре
технологии в действии. Мы создали элемент (его регистрация в DOM происходит за
кулисами, Polymer позаботится об этом), мы определили HTML-шаблон для нашего
элемента, который впоследствии будет использован в Shadow DOM, и, в заключение,
мы импортировали элемент, чтобы его использовать.

Конечно, в мире Polymer намного больше возможностей. Я рекомендую почитать
[документацию][4], чтобы узнать о них всё.

## Расширение существующих элементов

Одним из главных достоинств Web Components является возможность
расширять существующие элементы. И, так как созданные вами
компоненты являются простыми HTML-элементами, вы можете расширять их точно также, 
как нативные элементы. По большому счёту, нет никакой разницы
...
Новым здесь является элемент `<content>`, с его помощью
...
`ngTransclude`. Точки вставки, на самом деле, более сложны, но я сейчас не буду
...
повнимательней. Во первых, это элемент `<shadow>`. `<shadow>` — очень мощный, так
...
пользовательский элемент (через минуту мы узнаем зачем).
...
Вы также можете переопределять существующие методы и получать доступ к
свойствам с привязкой данных. Хотите узнать как? Об этом можно почитать
[здесь][6].
...
мышкой. Мы могли бы его использовать, применяя к существующим элементам, таким
как этот:
...
`extends="foo bar"`, верно? К сожалению, решения нет. Чтобы сделать немного
яснее:

**Невозможно расширить несколько элементов**

Вы ничего не найдете в официальной документации, ещё можете почитать это
[обсуждение на StackOverflow][7]. Однако есть несколько способов получить
*несколько* результатов, когда речь заходит о расширении множества элементов.

## Композиция вместо наследования

На первый взгляд, всегда лучше использовать композицию вместо наследования, чтобы
...
использующий `basic-icon`, и какой-нибудь `basic-link` (или что вам придет
в голову).
...
принципы, по большей части, основанные на композиции.
...
случаи, когда действительно хочется повторно использовать имеющуюся
...
Можно ли сделать это с помощью Polymer? Да, можно. Polymer поддерживает миксины.
...
`Platform.mixin()`. Если Polymer подключен, объект `Platform` доступен глобально, 
...
Сила Web Components позволяет сделать миксины доступными для
нескольких импортированных элементов. Даже если термин «Web Components» звучит
суперсложно, это всё ещё HTML, CSS и JavaScript. Те же инструменты, те же
правила.
